...
def parse_args():
    ...
    parser.add_argument('--PMLR_n_samples', type=int, default=11,
...
def main(args):
    ...
    dim_img = IMAGE_SIZE_celeba**2  # number of pixels for a celeba image
    ...
    train_total_data, train_size, _, test_data = celeba_data.prepare_celeba_data()
    ...
    if PRR:
        PRR = plot_utils.Plot_Reproduce_Performance(RESULTS_DIR, PRR_n_img_x, PRR_n_img_y, IMAGE_SIZE_celeba, IMAGE_SIZE_celeba, PRR_resize_factor)

        x_PRR = test_data[0:PRR.n_tot_imgs, :]

        x_PRR_img = x_PRR.reshape(PRR.n_tot_imgs, IMAGE_SIZE_celeba, IMAGE_SIZE_celeba)
    ...
    if PMLR and dim_z == 2:
        ...
        PMLR = plot_utils.Plot_Manifold_Learning_Result(RESULTS_DIR, PMLR_n_img_x, PMLR_n_img_y, IMAGE_SIZE_celeba, IMAGE_SIZE_celeba, PMLR_resize_factor, PMLR_z_range)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            kl_list.append(loss_divergence)
            ...
            if min_tot_loss > tot_loss or epoch+1 == n_epochs:
                ...
                if PRR:
                    ...
                    y_PRR_img = y_PRR.reshape(PRR.n_tot_imgs, IMAGE_SIZE_celeba, IMAGE_SIZE_celeba)
                    PRR.save_images(y_PRR_img, name="/PRR_epoch_%02d" %(epoch) + ".jpg")

                # Plot for manifold learning result
                if PMLR and dim_z == 2:
                    y_PMLR = sess.run(decoded, feed_dict={z_in: PMLR.z, keep_prob : 1})
                    y_PMLR_img = y_PMLR.reshape(PMLR.n_tot_imgs, IMAGE_SIZE_celeba, IMAGE_SIZE_celeba)
        ...
        plt.plot(epoch_list,kl_list, 'bs')
        plt.show()
